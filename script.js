...
let prev = null;
function randomNumber(start=0, end=5){
	const generate = Math.floor((Math.random() * end) + start)
	if(prev !== null){
		if(generate === prev){
...
function createNewTask(parent, task){
	...
	// Give random background color
	singleTask.css({"background":colors[randomNumber(0, 5)]});
